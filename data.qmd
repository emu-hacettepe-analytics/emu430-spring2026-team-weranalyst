...
final_data_clean %>%
  mutate(
    short_reason = case_when(
      str_detect(neden, "household|family") ~ "Family",
      str_detect(neden, "Better housing") ~ "Better Living",
      str_detect(neden, "Education") ~ "Education",
      str_detect(neden, "Assignment|change of job") ~ "Job Assignment",
      str_detect(neden, "Starting|finding a job") ~ "Finding a Job",
      str_detect(neden, "Unknown") ~ "Unknown",
      str_detect(neden, "Returning to family") ~ "Returning Home",
      str_detect(neden, "Buying a house") ~ "Buying a House",
      str_detect(neden, "Other") ~ "Other",
      str_detect(neden, "Health|care") ~ "Health",
      TRUE ~ neden
    )
  ) %>%
  group_by(short_reason) %>%
  summarise(total_migration = sum(gelen_sayi + giden_sayi, na.rm = TRUE)) %>%
  arrange(desc(total_migration)) %>%
  slice_head(n = 10) %>%
  ggplot(aes(x = reorder(short_reason, total_migration),
             y = total_migration)) +
  geom_col(fill = "#001f5b", width = 0.75) +
  coord_flip() +
  scale_y_continuous(labels = comma) +
